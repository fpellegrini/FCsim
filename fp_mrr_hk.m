...
if norm(iroi_seed(:)- iroi_tar(:),'fro')==0
    ...
    if half_m 
        ind_upper = find(triu(ones(nroi),1));
    else
        ind_upper = find(ones(nroi));
    end
    ...
    lab = label(:);
    tr = find(lab(ind_upper)==1);
    
    %% MRR
    for ii = 1:nints
       perfectSkill(ii) = 1/ii;
       noSkill(ii) = 1/(numel(lab(ind_upper))-ii+1);
    end
    perfectSkill = sum(perfectSkill)/nints;
    noSkill = sum(noSkill)/nints;

    [ss, idx] = sort(cc(ind_upper),'descend');
    ...
    if any(r1>nints)
        [~,~,~,pr]=perfcurve(lab(ind_upper),cc(ind_upper),1,'XCrit','reca','YCrit','prec');
    ...
    try
        ...
        d2_ = d2;
        d2_ = d2_ ./ sum(d2_(:));
        em1 = 1 - emd_hat_gd_metric_mex(d1, d2_, conndist);

        d2_ = d2;
        d2_(d2_ < prctile(d2_, 95)) = 0;
        d2_ = d2_ ./ sum(d2_(:));
        em2 = 1 - emd_hat_gd_metric_mex(d1, d2_, conndist);
