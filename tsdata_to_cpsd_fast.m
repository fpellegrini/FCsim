%% tsdata_to_wpli
%
% Estimate the weighted phase lag index from time series data
%
% <matlab:open('tsdata_to_wpli.m') code>
%
%% Syntax
%
%     S = tsdata_to_wpli(X,fres,method,window,noverlap,nw,ntapers)
%
%% Arguments
%
...
% _output_
%
%     wPLI          weighted phase lag index matrix
%
%% Description
%
...
%
%Modified by Franziska Pellegrini 2022
%%

function S = tsdata_to_cpsd_fast(X,fres,method,window,noverlap,nw,ntapers)
...
for ii = 1:n
    S(ii,:,:) = transpose(cpsd(X(:,ii),X,window,noverlap,nfft)); % cross-spectra
end
