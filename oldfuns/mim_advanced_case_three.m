...
for iroi = 1:nroi 
    clear ivoxelset ic
    ivoxelset = ind_roi_cortex{iroi};
    ichan = numel(ivoxelset)*2; 
    for ii = 1:numel(ivoxelset) 
        ic(((ii-1)*2)+1:((ii-1)*2)+2) = [((ivoxelset(ii)-1)*2)+1; ((ivoxelset(ii)-1)*2)+2] ;
        a=cat(1,a,[((ivoxelset(ii)-1)*2)+1; ((ivoxelset(ii)-1)*2)+2]);
    end
    
    for jroi = 1:nroi
        clear jvoxelset jc
        jvoxelset = ind_roi_cortex{jroi};
        jchan = numel(jvoxelset)*2;
        for jj = 1:numel(jvoxelset)
            jc(((jj-1)*2)+1:((jj-1)*2)+2) = [((jvoxelset(jj)-1)*2)+1; ((jvoxelset(jj)-1)*2)+2] ;
        end
        ...
        for ifq = 1:nfqA
            ...
            caainv=inv(real(cs_red{1})+regu*eye(ichan)*mean(diag(real(cs_red{1}))));
            cab=imag(cs_red{2});
            cbbinv=inv(real(cs_red{3})+regu*eye(jchan)*mean(diag(real(cs_red{3}))));
            X=cab*cbbinv*cab';
            % MIM Ewald Eq. 14
