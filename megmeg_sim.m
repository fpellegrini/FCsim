...
L = fp_get_lf(inverse);
L1 = squeeze(L(:,[inode1 inode2 rand_nodes],idir));
nmeg = size(L1,1);
id_meg_chan = 1:nmeg;
...
outname = sprintf('megmeg_sim_%s_trueandrand_mim_to_%d.nii',filtertype,inode2);
fp_data2nii(a,sources.pos,[],outname,id)

outname = sprintf('megmeg_sim_%s_trueandrand_mim_to_%d.nii',filtertype,inode1);
fp_data2nii(b,sources.pos,[],outname,id)
