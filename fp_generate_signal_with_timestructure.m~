...
if flag
    ...
    for ii = 1:params.iInt
        while iroi_seed(ii)==iroi_tar(ii)
...
% apply highpass
% signal_sources = filtfilt(bhigh, ahigh, signal_sources);
% signal_sources = signal_sources / norm(signal_sources, 'fro');
...
noise_sources = noise_sources / norm(noise_sources_f, 'fro');

% apply highpass
% noise_sources = filtfilt(bhigh, ahigh, noise_sources);
% noise_sources = noise_sources / norm(noise_sources, 'fro');


%% combine interacting and non-interacting sources 
...
signal_gt(:,noise_ind) = noise_sources;

% apply highpass
signal_gt = filtfilt(bhigh, ahigh, signal_gt);
signal_gt = signal_gt / norm(signal_gt, 'fro');
